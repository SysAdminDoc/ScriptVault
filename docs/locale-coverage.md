...
ScriptVault ships two independent translation surfaces:
...
The dashboard UI is currently English-only. The old dashboard-only inline
dictionary was removed because no dashboard HTML used `data-i18n` attributes.

`scripts/check-locales.mjs` is the CI gate that keeps the two active surfaces
...
2. **Inline-dict key drift** — `modules/i18n.js` should keep non-`en` locales
   aligned with `en`. Informational by default (so the existing key drift in
...
3. **Cross-source locale-set agreement** — the locales listed under
   `_locales/` and in `modules/i18n.js` should be the same set. A directory
   without runtime support, or a runtime locale missing from `_locales/`,
   surfaces as a "missing from" drift entry.
...
- `drifts[]` — per-drift entry, with `kind` ∈ {
  ...
  `cross-source-locale-mismatch` }, locale, plus `missing` / `orphaned` /
  `missingFrom` arrays.
...
1. Create `_locales/<code>/messages.json` cloned from `_locales/en` and
   ...
   matches.
2. Add the locale to the `translations` dictionary in `modules/i18n.js`.
3. Run `npm run locale:check` and resolve any drift entries.
4. Add a row to `tests/manifest-locales.test.js` if any new locale-specific
   length constraint needs pinning.

## Why not enforce inline-dict parity by default?

`modules/i18n.js` predates the `_locales/` directory and has historically
been the second-class i18n surface — runtime dictionaries serve features that
don't live in the manifest. The current dictionaries are missing keys per
locale that exist in `en`. Strict mode is opt-in until that backfill ships so
this gate doesn't block unrelated PRs; flip the CI workflow to
