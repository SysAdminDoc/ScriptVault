...
5. **Settings schema green:** `npm run settings:schema:check` confirms settings defaults, TypeScript fields, and dashboard save handlers are classified.
6. **CWS tooling green:** `npm run cws:check` confirms the installed `chrome-webstore-upload-cli` major, Node engine, removed flag usage, and credential names.
7. **CWS remote-code evidence green:** `npm run cws:remote-code:check` confirms `docs/cws-remote-code-compliance.md`, release/store-copy references, package script wiring, CI wiring, and source/package inputs do not contain forbidden remote-code execution patterns.
8. **Store permission copy green:** `npm run store-copy:check` confirms `manifest.json` and `manifest-firefox.json` permissions, host matches, web-accessible resources, sandbox pages, and AMO data-collection declarations are covered by `PRIVACY.md`, `docs/store-listing-copy.md`, and `AMO-SOURCE-README.md`.
9. **Locale lint green:** `tests/manifest-locales.test.js` passes - no locale's `extName` exceeds 75 chars, no `extDescription` exceeds 132 chars, all locales share the `en` key set.
10. **Version sources synced:** `manifest.json`, `manifest-firefox.json`, `package.json`, and `package-lock.json` all point to the same target version.
11. **Rollback drill green:** `npm run release:rollback-drill` proves the previous public `chrome.storage.local` snapshot survives the current storage migration safety window.
12. **Firefox AMO gate green:** `npm run firefox:package` exits with `web-ext lint: 0 errors, 0 notices` and writes `firefox-artifacts/scriptvault-firefox-vX.Y.Z.zip`, `firefox-artifacts/scriptvault-firefox-source-vX.Y.Z.zip`, and `firefox-artifacts/web-ext-lint.json`.
13. **Edge package gate green:** `npm run build:edge:check` writes `edge-artifacts/scriptvault-edge-vX.Y.Z.zip` and `edge-artifacts/edge-build-X.Y.Z.json`, and `npm run support:matrix:check` validates that the browser support matrix is tied to that report.
14. **Store status gate green:** `npm run release:store-status` validates rollback/trust/status command wiring, checks Firefox AMO lint/package evidence when present, and queries CWS API v2 `:fetchStatus` when `PUBLISHER_ID` plus `CWS_ACCESS_TOKEN` are provided.
15. **Release artifact parity green:** `npm run release:check` passes locally; after the tag and GitHub Release are created, `npm run release:check:public` must pass.
16. **CHANGELOG entry drafted:** one paragraph per shipped roadmap item, with `Phase X.Y` cross-references.
...
11. **Create or update GitHub Release:** attach `ScriptVault-vX.Y.Z.zip`, `firefox-artifacts/*`, `edge-artifacts/*`, `release-artifacts/*`, and the GitHub Actions attestation links; mark it as latest for normal production releases.
...
19. **Edge Partner Center upload:** upload `edge-artifacts/scriptvault-edge-vX.Y.Z.zip` manually after the Chrome/Firefox release artifacts are validated. Attach or cite `edge-artifacts/edge-build-X.Y.Z.json` in the submission notes, and use `docs/edge-submission.md` for privacy, permission, and remote-code declarations.
20. **Record store status:** rerun `npm run release:store-status` with `CWS_ACCESS_TOKEN` when available and attach the output to release notes or the release checklist.
