...
- `npm run smoke:firefox` runs a geckodriver-based temporary sideload smoke against Firefox Developer Edition/Nightly 140+, opens the dashboard and popup, saves/toggles a smoke userscript through the extension message path, verifies it runs on a local target page, validates WebDAV sync against a local fixture, imports Chrome-shaped JSON/ZIP backup fixtures, imports a 26-script quota fixture, and verifies trash restore after a profile-backed Firefox restart.
