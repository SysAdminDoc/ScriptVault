...
- `docs/amo-vendored-libraries.md` lists the package name, version, license,
  ...
  status for every shipped third-party byte. Chrome/Edge-only CodeMirror and
  Monaco rows remain documented even though the AMO build omits those files;
  the provenance check validates their hashes whenever the generated assets
  are present.
...
- Optional `downloads`: supports user-triggered downloads, exports, backups,
  and reports; requested at runtime when a script declares `@grant GM_download`.
