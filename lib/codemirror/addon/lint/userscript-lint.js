...
(function(mod) {
  ...
  const KNOWN_DIRECTIVES = [
    ...
    'connect', 'antifeature', 'unwrap', 'nodownload', 'delay', 'inject-into', 'sandbox',
    'compatible', 'incompatible', 'license', 'copyright', 'contributor',
    'contributionURL', 'webRequest',
    'tag', 'run-in', 'top-level-await', 'priority'
  ];
  ...
  function lintUserscript(text, options, cm) {
      ...
      if (directive === 'priority') {
        if (value && !/^-?\d+$/.test(value)) {
          annotations.push({
            from: CodeMirror.Pos(i, line.indexOf(value)),
            to: CodeMirror.Pos(i, line.length),
            message: '@priority must be an integer (e.g., 1, 10, -5). Higher values run first.',
            severity: 'error'
          });
        }
      }
    ...
    const matchValues = foundDirectives.match.map(m => m.value);
    const seenMatch = {};
    for (const m of foundDirectives.match) {
      if (seenMatch[m.value]) {
        annotations.push({
          from: CodeMirror.Pos(m.line, 0),
          to: CodeMirror.Pos(m.line, lines[m.line].length),
          message: `Duplicate @match pattern: ${m.value}`,
          severity: 'warning'
        });
      }
      seenMatch[m.value] = true;
    }

    // Check for duplicate @grant entries
    const seenGrant = {};
    for (const g of foundDirectives.grant) {
      if (seenGrant[g.value]) {
        annotations.push({
          from: CodeMirror.Pos(g.line, 0),
          to: CodeMirror.Pos(g.line, lines[g.line].length),
          message: `Duplicate @grant: ${g.value}`,
          severity: 'warning'
        });
      }
      seenGrant[g.value] = true;
    }

    // Warn on overly broad @match patterns
    const broadPatterns = ['*://*/*', '<all_urls>', 'http://*/*', 'https://*/*'];
    for (const m of foundDirectives.match) {
      if (broadPatterns.includes(m.value)) {
        annotations.push({
          from: CodeMirror.Pos(m.line, 0),
          to: CodeMirror.Pos(m.line, lines[m.line].length),
          message: 'Very broad match pattern — script will run on all/most pages. Consider narrowing.',
          severity: 'info'
        });
      }
    }
    ...
    for (let i = metaStartLine + 1; i < metaEndLine; i++) {
      ...
      if (dm) {
        if (dm[1] === 'version') hasVersion = true;
        if (dm[1] === 'description') hasDescription = true;
      }
    }
    if (!hasVersion) {
      annotations.push({
        from: CodeMirror.Pos(metaEndLine, 0),
        to: CodeMirror.Pos(metaEndLine, lines[metaEndLine].length),
        message: 'Missing @version — recommended for update tracking',
        severity: 'info'
      });
    }
    if (!hasDescription) {
      annotations.push({
        from: CodeMirror.Pos(metaEndLine, 0),
        to: CodeMirror.Pos(metaEndLine, lines[metaEndLine].length),
        message: 'Missing @description — recommended for script discovery',
        severity: 'info'
      });
    }

    // Duplicate @require URLs
    const seenRequire = {};
    for (const r of foundDirectives.require) {
      const url = r.value.split('#')[0]; // ignore hash for comparison
      if (seenRequire[url]) {
        annotations.push({
          from: CodeMirror.Pos(r.line, 0),
          to: CodeMirror.Pos(r.line, lines[r.line].length),
          message: 'Duplicate @require URL',
          severity: 'warning'
        });
      }
      seenRequire[url] = true;
    }
