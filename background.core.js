...
const UpdateSystem = {
   ...
   */
  async _recordUpdateHostFailure(script, failure, updateUrl) {
    if (!script) return;
...
function audioWatchKeysForTab(tabId) {
...
function removeAudioWatchKeysForTab(tabId) {
  const watched = self._audioWatchedTabs;
...
function getSettingsImportType(key) {
...
function isSettingsImportValue(value, type) {
    ...
    return await _runExclusiveScriptOperation(id, async () => {
      ...
      if (!Array.isArray(parsed.meta.match) || parsed.meta.match.length === 0) {
        ...
        const fallbackMatches = Array.isArray(requestedFallbacks)
          ? requestedFallbacks
              .filter(match => typeof match === 'string' && match.trim())
...
backgroundActionRegistry.registerHandlers(RuntimeActionHandler.createRuntimeActionHandlers({
  ...
  previewUserStyle: (code, tabId, values, colorScheme) => typeof UserStylesEngine !== 'undefined'
  ...
  installUserStyle: async (code, enabled) => {
    ...
    try {
      ...
      const response = { success: true, id, name: parsed.meta?.name || id };
...
backgroundActionRegistry.registerHandlers(DiagnosticsActionHandler.createDiagnosticsActionHandlers({
  ...
  reportDocumentReady: (url, sender, reportedDocumentId) => {
    ...
    if (
      ...
      UserStylesEngine.onDocumentCommitted(sender.tab.id, url || sender?.tab?.url, documentId)
        .catch(error => console.error('[ScriptVault] UserCSS document identity handoff failed:', error));
    }
    return { success: true };
  },
 ...
 */
async function trashScriptForSync(script, reason = 'sync') {
...
async function analyzeSyncedScriptCode(code) {
 ...
 */
async function notifySyncedScriptReview(details) {
...
async function executeWrappedScriptInTab(tabId, wrappedCode, wantsPageContext, scriptId = undefined) {
  if (typeof chrome.userScripts?.execute === 'function') {
...
function _normalizeUserStylesDocumentId(value) {
...
if (chrome.webNavigation?.onCommitted?.addListener) {
  chrome.webNavigation.onCommitted.addListener(async (details) => {
    ...
    UserStylesEngine.onDocumentCommitted(details.tabId, details.url, documentId)
      .catch(e => console.error('[ScriptVault] UserCSS document-start registration error:', e));
  });
}
...
async function _storePendingInstall(storageKey, payload, options = {}) {
...
async function verifyManagedScriptPolicyEntry(item, code) {
...
function _isFileSchemePattern(pattern) {
  const raw = String(pattern || '').trim().toLowerCase();
  return raw.startsWith('file://') || raw.startsWith('file:/*') || raw === 'file:///*';
}

async function _hasFileScopedScripts() {
  try {
    const scripts = await ScriptStorage.getAll();
    return (Array.isArray(scripts) ? scripts : []).some(s => {
      const patterns = [
...
async function probeFileSchemeAccess() {
  let supported = false;
  let allowed = false;
  try {
    const g = globalThis;
    const ext = (g.browser && g.browser.extension)
      ? g.browser.extension
      : ((typeof chrome !== 'undefined' && (chrome)?.extension) ? (chrome).extension : null);
...
function deniedHostExcludePatterns(rawHost) {
