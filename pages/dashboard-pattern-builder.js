...
const PatternBuilder = (() => {
  ...
  function renderVisualBuilder() {
    ...
    const wildcardToggle = el('button', {
      class: 'pb-toggle' + (_state.hostWildcard ? ' active' : ''),
      title: 'Subdomain wildcard (*. prefix)',
      'aria-pressed': String(_state.hostWildcard),
      'aria-label': 'Toggle subdomain wildcard prefix',
    });
    ...
    _state.pathSegments.forEach((seg, i) => {
      ...
      const removeBtn = el('button', { class: 'pb-remove-btn', text: '\u00d7', title: 'Remove segment', 'aria-label': `Remove path segment ${i + 1}` });
  ...
  function renderTestUrls() {
    ...
    const input = el('input', {
      class: 'pb-input',
      type: 'url',
      name: 'patternBuilderTestUrl',
      autocomplete: 'off',
      spellcheck: 'false',
      placeholder: 'Paste a URL & press Enter to test…',
    });
  ...
  function renderPresets() {
    ...
    PRESETS.forEach(preset => {
      const chip = el('button', { class: 'pb-preset-chip', text: preset.label });
