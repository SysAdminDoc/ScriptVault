...
async function analyzeAsync(code: string): Promise<AnalysisResult> {
  try {
    ...
    const result: AnalysisResult | undefined = await chrome.runtime.sendMessage({ type: 'offscreen_analyze', code });
...
function calculateEntropy(str: string): number {
  ...
  for (const count of Object.values(freq)) { const p: number = count / len; entropy -= p * Math.log2(p); }
  return entropy;
}
