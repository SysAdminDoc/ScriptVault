...
>
> **Roadmap version:** Round 40 - Monaco ESM sandbox 2026-06-06.
...
> **Test suite:** 1452 Vitest cases green; `npm audit --audit-level=high --omit=optional` clean; 27/27 TS-promoted runtime entries; 0 mirrored; 0 divergent.
...
- **Current cycle:** Round 40 Cycle 76 switched the Chromium sandbox from deprecated AMD loading to packaged local ESM loading, removed the AMD copy step from the build, and updated the package contract to reject AMD regressions.
- **Next implementation angle:** Cycle 77 should continue X-4 with a browser-level ESM editor smoke and fallback proof. If live extension profiles are blocked, add the closest deterministic harness that loads the sandbox module path and proves missing-bundle fallback routing.
