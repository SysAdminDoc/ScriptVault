...
- [ ] P2 — Surface per-source health in the Find Scripts panel
  Why: `RemoteResponseClassifier` now distinguishes host-challenge / transport / http-status / not-a-userscript for the UPDATE path, and the update checker records `_updateLastFailure` on the script. The Find Scripts panel still shows an empty result list when a catalog host is down or challenged, so the user cannot tell "no matches" from "api.greasyfork.org is unreachable".
  Evidence: `src/background/remote-response-classifier.ts` shipped 2026-08-08 and is wired only into `UpdateSystem.checkForUpdates`; discovery still hardcodes https://api.greasyfork.org/en/scripts.json and https://openuserjs.org/api/script/list in pages/dashboard.js with no per-source status; greasyfork#1553 (Cloudflare challenge) and #1561 (expired certs) are the live failure modes.
  Touches: pages/dashboard.js Find Scripts panel, src/background/find-script-sources.ts, src/locales/en.json, tests/ find-scripts coverage.
  Acceptance: each catalog source shows its own state (ok / challenged / unreachable / http error) with the classifier's message, and an empty list is distinguishable from a failed source; a test drives a challenged source and a transport failure.
  Complexity: M
