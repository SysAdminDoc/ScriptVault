...
>
> **Roadmap version:** Round 35 - action behavior select validation slice 2026-06-06.
...
> **Source floor:** 400+ external URLs across Rounds 1-35. Every Now/Next item carries source IDs from the Appendix.
>
> Last researched: Round 35 - 2026-06-06.
...
- **Current cycle:** Round 35 Cycle 67 continued N-1 Settings Schema Parity and Accessible Validation. Default tab type, local file, cookie, communication, SRI, include, @connect, incognito, page filter, block severity, strict mode, and top-level await selects now have schema validation metadata, accessible error nodes, and save-blocking allowed-option checks. Focused dashboard a11y/schema tests, settings schema check, TS runtime check, audit, full check suite, build, CWS scan, and `git diff --check` passed.
- **Next implementation angle:** Cycle 68 should continue N-1 by applying allowed-option validation to the remaining general/action/editor/download selects that still lack validation metadata: badge info, blacklist source, config mode, download mode, editor theme, highlight matches, indent style, key mapping, logging level, popup columns, script order, search integration, tab mode, and trash mode; then continue export/backup setting constraints with focused dashboard a11y/schema coverage.
