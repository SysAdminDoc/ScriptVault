...
- [x] P1 — Continue TypeScript authoritative-source promotion until `background.core.js` is generated from TS
  - Why: runtime/TS mirror drift is collapsed for the existing concatenated build. `ts-source-promotion.json` now tracks 26 promoted entries, 0 mirrored entries, and 0 intentionally divergent runtime files.
  - Acceptance: `ts-source-promotion.json` reaches 0 mirrored; `background.core.js` is generated from TS source.
  - Progress: 2026-06-03 promoted `bg/workspaces.js` from `src/bg/workspaces.ts`, preserving the `_initPromise` cold-start guard. Promoted `bg/signing.js` from `src/bg/signing.ts`, preserving the null signature guard and runtime `SettingsManager` global contract. Promoted `bg/esm-bundler.js` from `src/bg/esm-bundler.ts`, preserving the runtime `fetchRequireScript` global and `self.ESMUserscriptBundler` alias. Promoted `modules/public-api.js` from `src/modules/public-api.ts`, preserving generated IDs, permissions access, bounded web installs, and webhook guards. Promoted `modules/backup-scheduler.js` from `src/modules/backup-scheduler.ts`, preserving backup verification, restore receipts, rollback receipts, retention caps, and runtime-global storage contracts. Promoted `modules/sync-easycloud.js` from `src/modules/sync-easycloud.ts`, preserving alarm-backed debounce, runtime script refresh hooks, Drive timeouts, offline queue handling, and chrome.identity token-cache semantics. Promoted `modules/sync-providers.js` from `src/modules/sync-providers.ts`, preserving OAuth timeout probes, WebDAV/Drive/Dropbox/OneDrive providers, S3 SigV4 signing, token disclosure metadata, and `self.CloudSyncProviders`. Promoted `background.core.js` from the raw TypeScript bridge source `src/background/core.ts`, preserving top-level helper visibility for existing runtime tests while making the core runtime generated output.
  - Source: docs/archive/TODO.md F-1 (ROADMAP Larger Bets).
- [x] P1 — Complete release trust pipeline (rollback rehearsal automation, CWS/AMO status checks)
...
- [x] P2 — Sigstore `@require-provenance` implementation
  - Why: design-complete supply-chain provenance for `@require`.
  - Progress: 2026-06-04 shipped Phase A parser/storage foundation, Phase B bundle parsing, Phase C message-signature verification, Phase D Fulcio root/validity checks, dashboard/update review surfacing, install-dialog verified-author preview, fail-closed save/update enforcement for declared provenance failures, and `docs/provenance-author-guide.md`. `@require-provenance` and `@require-identity` now persist as ordered metadata arrays, `modules/sigstore-bundle-parser.js` validates Sigstore v0.3 JSON, and `modules/sigstore-bundle-verifier.js` verifies message-signature bundles by hashing the `@require` body, extracting the leaf certificate key, validating DER/raw ECDSA signatures, matching URI SAN plus Fulcio issuer extension against `@require-identity`, and chaining the leaf to the bundled Fulcio v1 root with verifier-time certificate validity checks. Trust receipts now record `signature-verified`, `signature-failed`, `root-verification-failed`, `bundle-unavailable`, or `unsupported-bundle` outcomes when both dependency and bundle fetchers are available; pending updates with failed or incomplete provenance are marked for review, dashboard trust-receipt/recent-update views show per-`@require` provenance rows, and the install review card previews provenance before save. RFC3161/Rekor timestamp proof remains Phase 2 defense-in-depth.
  - Source: docs/archive/TODO.md F-4 (`docs/require-provenance-design.md`).
- [x] P2 — Module-mode service worker (Chrome 124+ `"type":"module"` background)
  - Progress: 2026-06-04 Chrome `manifest.json` now declares `background.type: "module"` for the existing single-file `background.js` service worker. Firefox remains on the generated event-page `background.scripts` shape, Edge preserves the Chromium module worker manifest, and `tests/module-service-worker.test.js` pins the Chrome floor plus no global `importScripts()` loader calls / static import / export usage in the built worker.
...
- [x] P2 — Phase 3: Firefox DNR, `@require` SRI, and Ed25519 parity validation
...
- [x] 🔬 `optional-dependency-reach-gate-2026-06-04` - rechecked the
  ...
  peer-optional edges from toolchain/browser-test packages. A static
  import/require scan over 116 shipped extension files found zero imports of
  those optional-like package names; the only loose string hits were DOM
  `<canvas>` usage in dashboard modules, not the npm `canvas` package. That
  makes the exemption plausible today, but npm documents that omitted optional
...
- [x] 🤖 🔧 🔬 P2 — Add a Firefox for Android smoke gate or remove the Android compatibility claim before AMO listing
  ...
  - Progress: 2026-06-04 chose option B until a real Android device/emulator smoke exists. `manifest-firefox.json` no longer declares `gecko_android`, the generated support matrix marks Firefox for Android as deferred rather than an AMO compatibility target, and `FIREFOX-PORT.md`/pipeline docs record that Android is intentionally not claimed.
  - Verification: `npm run support:matrix`; `npm run support:matrix:check`; `npm test -- tests/firefox-package.test.js`.
  - Complexity: M

### Firefox AMO source-review quality

- [x] 🤖 🔬 P2 — Add reviewer-reproducible provenance for packaged minified third-party libraries
...
- [x] 🤖 🔬 P1 — Add a CWS remote-hosted-code compliance packet and package scan
...
- [x] 🤖 🔬 P2 — Tie Edge support claims to the generated Edge artifact and CI evidence
  ...
  - Complexity: S
- [x] 🤖 🔬 P2 — Add a dedicated Edge browser smoke before raising Edge support beyond package-ready
  ...
  - Progress: 2026-06-05 added `npm run smoke:edge`, a Puppeteer-based Microsoft Edge sideload harness that builds the Edge package, loads `build-edge/`, opens dashboard and popup pages, validates `chrome.userScripts` availability, saves/toggles a smoke userscript, verifies it runs on a local `127.0.0.1` target page, captures extension page/service-worker errors, and writes `edge-artifacts/edge-smoke-<version>.json`. The Edge build report now advertises the smoke command/evidence path while the support matrix remains conservative about Partner Center publication and CI smoke status.
  - Verification: `npm run smoke:edge`; `npm run build:edge:check`; `npm test -- tests/edge-smoke.test.js tests/edge-build.test.js`; `npm run support:matrix`; `npm run support:matrix:check`; `npm run readme:check`; `npm run store-copy:check`; `npm run check`; `git diff --check`.
  - Complexity: M
...
- [x] 🤖 🔬 P1 — Apply internal-host guard to user-configured sync provider endpoints
  ...
  - Acceptance: WebDAV `test`/`upload`/`download` and S3 `test`/`upload`/`download` reject loopback, link-local, metadata, RFC1918, CGNAT, ULA, and blocked redirect targets by default; a visible explicit local/private endpoint opt-in permits intended LAN/self-hosted endpoints with visible risk copy; fixed-host OAuth providers keep their existing behavior.
  - Progress: 2026-06-04 WebDAV and S3 sync requests now run shared internal-host preflight and post-fetch redirect checks before response bodies are read. The default blocks localhost, loopback, link-local, private IPv4, CGNAT, metadata endpoints, and private IPv6/ULA endpoints; a visible advanced `allowInternalSyncEndpoints` checkbox in Userscript Sync permits deliberate LAN/self-hosted WebDAV or S3-compatible endpoints.
  ...
  - Verification: `npm run build:bg`; `npm run typecheck`; `npm test -- tests/source-sync-providers.test.js tests/s3-sync-provider.test.js tests/source-hardening-parity.test.js tests/storage.test.js tests/internal-host-guard.test.js tests/dashboard-a11y.test.js`; `npm run test:a11y`; `npm run ts-runtime:check`; `npm run ts-source:check`; `npm run store-copy:check`; `npm run readme:check`; `npm run check`; `git diff --check`.
  - Complexity: M
- [x] 🤖 🔬 P1 — Redact or separately gate credential-bearing settings in vault exports and scheduled backups
  ...
  - Progress: 2026-06-04 JSON/cloud exports and managed backup ZIPs now redact WebDAV credentials, OAuth tokens, and S3 keys by default. Explicit credential inclusion is separate from the normal settings checkbox, scheduled backups have their own off-by-default credential switch, backup ZIPs stamp `global-settings.metadata.json`, and restore/import only writes credential keys when both archive metadata and the user opt-in are present.
  - Verification: `npm run ts-runtime:generate`; `npm run build:bg`; `npm run typecheck`; `npm test -- tests/runtime-import-export.test.js tests/source-backup-modules.test.js tests/backup-scheduler.test.js tests/backup-receipts.test.js tests/dashboard-a11y.test.js tests/source-hardening-parity.test.js`; `npm run ts-runtime:check`; `npm run ts-source:check`; `npm run store-copy:check`; `npm run readme:check`; `npm run test:a11y`; `npm run check`; `git diff --check`.
  - Complexity: M
- [x] 🤖 🔬 P1 — Bound backup ZIP/JSON intake before full decompression and registration
...
- [x] 🤖 🔬 P1 — Optional client-side E2E encryption for cloud sync
  ...
  - Progress: 2026-06-04 added `SyncCrypto` and a v2 encrypted sync-envelope format. CloudSync and EasyCloud now decrypt encrypted remote v2 envelopes before merge, still accept plaintext v1 envelopes, and encrypt sanitized upload envelopes when `syncEncryptionEnabled` plus a local passphrase are configured. The dashboard exposes the opt-in and passphrase field, and export/backup credential redaction treats `syncEncryptionPassphrase` as a credential.
  - Verification: `npm run ts-runtime:generate`; `npm run typecheck`; `npm test -- tests/sync-crypto.test.js tests/source-cloud-sync.test.js tests/source-sync-easycloud.test.js tests/source-hardening-parity.test.js`; `npm run build:bg`; `npm run check`; `npm run ts-runtime:check`; `npm run ts-source:check`; `npm run store-copy:check`; `npm run readme:check`; `npm run smoke:dashboard`; `npm run cws:remote-code:check`; `git diff --check`.
...
- [x] 🤖 🔬 P2 — Add host-permission recovery prompts and an optional narrow-host prototype
  ...
  - Progress: 2026-06-04 added a `HostPermissionPatterns` runtime helper plus background `getHostPermissionStatus` / `queueHostAccessRequest` diagnostics. Popup setup warnings, the side panel, and dashboard Runtime Repair now show blocked matching scripts when browser site access is withheld; Chrome queues `permissions.addHostAccessRequest` when present and extension pages fall back to `permissions.request({ origins })`. Permission grant/revoke listeners refresh badge state and open diagnostics. The default Chrome/Firefox manifests still require `<all_urls>`, but `npm run host-permissions:prototype` now generates a gated optional-host report for `http://*/*` / `https://*/*` so future lower-warning manifest work has a reproducible review artifact before any shipping change.
  - Verification: `npm run ts-runtime:generate`; `npm run build:bg`; `npx vitest run tests/host-permissions-recovery.test.js tests/dashboard-a11y.test.js tests/popup-a11y.test.js tests/accessibility-surface-pass.test.js tests/source-hardening-parity.test.js --pool=vmThreads --maxWorkers=1 --reporter=dot`; `npm run typecheck`; `npm run ts-runtime:check`; `npm run ts-source:check`; `npm run gm-types:check`; `npm run host-permissions:prototype`; `npm run host-permissions:prototype:check`; `npm run build:edge:check`; `npm run support:matrix`; `npm run support:matrix:check`; `npm run store-copy:check`; `npm run readme:check`; `npm run cws:remote-code:check`; `npm run test:a11y`; `npm run check`; `npm run smoke:dashboard`; `npm run smoke:firefox`; `git diff --check`.
  - Complexity: L
...
- [x] P1 — Correct `@crontab` next-fire engine
...
- [x] P1 — Structured `@antifeature` parsing + install label + row chip
  ...
  - Progress: 2026-06-04 added structured `@antifeature` parsing in the source parser, raw service-worker parser, install-page parser, and public API install parser. Dashboard rows now show an amber antifeature chip, and install warnings label all six Greasy Fork-recognized types while preserving optional descriptions and locales.
  - Verification: `node --check pages/install.js`; `node --check pages/dashboard.js`; `node --check modules/public-api.js`; `node --check background.core.js`; `npm test -- tests/script-antifeature.test.js tests/parser.test.js`; `npm run typecheck`; `npm run ts-runtime:check`; `npm run ts-source:check`; `npm run check`; `npm run readme:check`; `npm run store-copy:check`; `npm run cws:remote-code:check`; `npm run smoke:dashboard`; `git diff --check`.
  - Source: docs/archive/RESEARCH_FEATURE_PLAN_PASS3.md EI-3.
- [x] P2 — Wire the built npm/ESM `@require` resolver
  ...
  - Acceptance: `npm:` `@require` specs resolve through the promoted NpmResolver, execute the exact bytes hashed for SRI, cache under the npm spec and final CDN URL, and reject non-HTTPS or internal-host redirected CDN responses before caching.
  - Progress: 2026-06-04 added `NpmResolver.resolveWithCode()`, wired `fetchRequireScript()` / the generated background core to use it for `npm:` specs, preserved cache-bypassing TOFU/provenance behavior, and applied `InternalHostGuard` pre/post checks to npm CDN fetches.
  - Verification: `npm run ts-runtime:generate`; `npm run build:bg`; `npm run typecheck`; `npm run ts-runtime:check`; `npm run ts-source:check`; `node --check modules/npm-resolve.js`; `node --check background.core.js`; `node --check background.js`; focused Vitest passed `tests/npm-resolve.test.js tests/source-modules.test.js tests/audit-hardening-2026-06.test.js --pool=vmThreads --maxWorkers=1 --reporter=dot` (77 tests); `npm run check` passed (115 files, 1337 tests); `npm run store-copy:check`; `npm run readme:check`; `npm run cws:remote-code:check`; `git diff --check`.
  - Complexity: S
...
- [x] P2 — Align CI Node setup, package-manager metadata, and engine enforcement with the Node 21.2.0 floor
  ...
  - Progress: 2026-06-05 aligned the toolchain contract on Node 24.16.0+ / npm 11.13.0+. `package.json` now declares `packageManager`, Node and npm engines; `.node-version`, `.nvmrc`, and `.npmrc` provide local install metadata and `engine-strict=true`; CI uses `node-version-file: .node-version` and runs `npm run toolchain:check`; the CWS tooling check now compares current Node against the project engine instead of a hard-coded Node 20 floor; contributor and release docs reference the shared contract.
  - Verification: `npm install --package-lock-only`; `npm run toolchain:check`; `npx vitest run tests/toolchain-contract.test.js --environment=node --reporter=verbose`; `npm audit --audit-level=high --omit=optional`; `npm run cws:check`; `npm run check`.
  - Complexity: S
...
- [x] P0 — Bump `web-ext` to ≥10.3.0 to clear CVE-2026-44705 (`tmp` path traversal) and unblock CI
  ...
  - Complexity: S
- [x] P1 — Coverage measures only test-imported files (`all:false`, no threshold) — gate the runtime
  ...
  - Progress: 2026-06-05 shipped the source-aligned coverage gate. `vitest.config.mjs` now includes `src/background/**`, `src/bg/**`, `src/modules/**`, `src/shared/**`, and `src/storage/**`, emits `json-summary`, and enforces the initial measured floor (10% lines/functions/statements, 5% branches). `npm run test:cov` now chains `scripts/check-coverage-sources.mjs`, which fails if `coverage-summary.json` omits any authoritative source-root file or promoted source from `ts-source-promotion.json`; CI runs the same gate after the normal test step. The dashboard card-view fixture now dispatches the image error event from the jsdom window realm so coverage runs preserve the browser fallback contract.
  - Verification: local mirror at `C:\tmp\ScriptVault-verify`; `npm ci` (0 vulnerabilities); `npm run typecheck`; `npm run test:cov` (118 files, 1354 tests, coverage all files 37.94% statements / 27.97% branches / 48.32% functions / 40.12% lines; guard OK for 48 authoritative sources and 28 promoted sources).
  - Complexity: M
- [x] P1 — Automate dependency freshness (Dependabot/Renovate) instead of reactive audit
  ...
  - Progress: 2026-06-05 added `.github/dependabot.yml` for weekly npm and GitHub Actions version updates. npm minor/patch dev-tooling updates are grouped into test tooling, browser test tooling, extension release tooling, build tooling, and editor runtime buckets; major updates remain outside those groups as separate PRs. GitHub Actions minor/patch updates are grouped to keep future action pin maintenance visible without blocking SHA-pinning work. `docs/dependency-audit-policy.md` now documents the freshness policy and no broad ignore rules.
  - Verification: `npm outdated --json` in `C:\tmp\ScriptVault-verify` confirmed the current direct devDependency drift; `npx vitest run tests/dependabot-config.test.js --environment=node --reporter=verbose`; `npm audit --audit-level=high --omit=optional`.
  - Complexity: S
- [x] P1 — Pin GitHub Actions to commit SHAs (supply-chain hardening of the release pipeline)
...
- Remaining non-release-trust guardrails: unified internal-host policy and sync token revoke/status UI. Artifact signing, SBOM, package diff, rollback rehearsal, AMO data-collection manifest declaration, source ZIP review artifact, and restore receipts now have dedicated gates or shipped evidence.
...
- Test gaps: no current focused tests for recent Gist storage rejection fix, empty-grant deny behavior, credentialed AMO signing, or Chrome 138 setup copy.
...
- [x] P2 - Evaluate generated multi-target manifest/build system
  ...
  - Status: Shipped 2026-05-24 as `docs/manifest-generation-design.md`. Measured drift between `manifest.json` and `manifest-firefox.json` (108 diff lines across 8 sections) and the per-release maintenance cost (~10 minutes plus the standing risk of forgetting a section). Evaluated three options — WXT, a thin generator script, or status quo — against migration cost, drift surface, build complexity, loss of the inlined `background.js` contract, Edge add-on readiness, contributor onboarding, HMR availability, and CI risk. Chose **Option B (thin generator helper)**: a Node script that produces `manifest-firefox.json` from `manifest.json` plus a small declarative transformation file, keeping the existing source layout / esbuild pipeline / test suite intact. Existing `build-firefox.sh` is the proof-of-concept that Chrome + Firefox builds already come from one source tree — only the manifest transformation needed externalising. Follow-up implementation shipped 2026-05-24 via `scripts/generate-manifest-firefox.mjs` + `manifest-firefox.transformations.json`: the Firefox profile round-trips `manifest-firefox.json` byte-for-byte and gates `build-firefox.sh` on drift, while the Edge profile now stages `build-edge/manifest.json` from the same generator instead of inline transforms. Generated manifests remain committed pending maintainer sign-off before any gitignore change.
...
- [x] P2 - Add S3-compatible sync provider
  ...
  - Status: Shipped 2026-05-24. Added the `s3` provider to `CloudSyncProviders` in both `modules/sync-providers.js` (runtime, full SigV4 implementation) and `src/modules/sync-providers.ts` (TS mirror with thin pass-through + structural validation). Provider exposes `validate`, `upload`, `download`, `test`, `getStatus`, `disconnect`, `getStorageDisclosure`, and an internal `_signRequest` AWS Signature v4 signer that uses Web Crypto SubtleCrypto (HMAC-SHA256 + SHA-256, no SDK). URL construction handles both virtual-host style (AWS) and path-style (Cloudflare R2, MinIO, Backblaze B2) automatically; `s3PathStyle: true` overrides on AWS for users behind path-style proxies. `validate()` returns a structured `{ valid, errors[] }` for per-field UI feedback covering endpoint URL scheme/path, region presence, bucket name validity, and credential presence. `test()` HEAD treats 404 as success (bucket reachable, no backup yet). Settings UI: dashboard Userscript Sync panel gained a new "S3-compatible" provider option plus a six-field settings block (endpoint/region/bucket/access-key/secret/object-key). New `s3Endpoint`/`s3Region`/`s3Bucket`/`s3AccessKeyId`/`s3SecretKey`/`s3ObjectKey` fields added to `Settings` type, `SyncProvider` union, and `src/config/settings-defaults.json`. Mock-server test coverage: `tests/s3-sync-provider.test.js` (21 cases — validation, URL construction across path-style/virtual-host/forced-path, SigV4 header shape + deterministic SHA-256, upload PUT with Authorization header, download GET with 200/404/403 handling, test() HEAD with success/404-as-success/401-error, disclosure shape, status states, disconnect clears all). Verification: `npx vitest run tests/s3-sync-provider.test.js tests/source-sync-providers.test.js tests/source-cloud-sync.test.js tests/sync-cockpit.test.js --pool=vmThreads --maxWorkers=1` passed (44 tests across 4 files); `npm run typecheck` clean; `npm run build:bg` clean (background.js 21,782 lines).
...
- [x] P3 - Research ESM userscript and local development modes
  ...
  - Status: Shipped 2026-05-24 as `docs/esm-userscript-research.md`. Identifies two viable injection shapes (install-time pre-bundling vs page-injected `<script type="module">`) and rejects the second on isolation/security grounds. Maps the CSP gate (`connect-src`, page `script-src`, MV3 extension CSP) and demonstrates pre-bundling sidesteps both page and extension CSP because the bundle is stored as a plain JS string in chrome.storage. Calls out the transitive-import audit gates (SRI / host allowlist / bounded fetch) the bundler must reuse, and the static-only rewriting rule (Acorn AST already shipped in offscreen.js). Local-dev mode evaluation rejects filesystem watchers (no MV3 API) and chooses an SSE-from-localhost loop reusing the existing install/update plumbing under a new Developer Mode panel. Permanently rejects runtime `import()` under MV3 CSP. Migration strategy is phased R-1 → R-5 with every phase gated behind off-by-default settings. R-1 implementation shipped 2026-05-24: `experimentalESMUserscripts` defaults false; `@module 1` and `@inject-into module` mark metadata as ESM; `bg/esm-bundler.js` / `src/bg/esm-bundler.ts` statically rewrite imports via offscreen Acorn, recursively fetch dependencies through the existing `fetchRequireScript` path, and reject dynamic `import()`; install/update paths bundle only when the flag is enabled. Verification landed in `tests/esm-bundler.test.js` and `tests/esm-csp.test.js`.

- [x] P3 - Expand userstyle support after Firefox/userstyle CSP tests
  ...
  - Status: Shipped 2026-05-24. Added `tests/userstyle-compat-fixtures.test.js` (18 cases) exercising `parseUserCSS` against seven representative fixtures: no-variable baseline + missing-metadata error path, multiple `@var color`, `@var select` with pipe-separated options, the other four `@var` types (`text`/`number`/`checkbox`/`range`), `@-moz-document` blocks preserved verbatim, multi-section bodies that reference the same variable from multiple selectors, and Japanese (`日本語スタイル`) labels with non-ASCII default values. Pinned that `@var range` exposes `min`/`max`/`step` under `options` (Stylus convention) and `default` at the top level. Documented the Chrome/Firefox compatibility baseline at `docs/userstyle-compat.md` including the parser surface, the runtime injection parity matrix (`chrome.scripting.insertCSS` works identically across MV3 in both engines, `@-moz-document` is preserved verbatim, ScriptVault matches patterns pre-injection so behavior is engine-agnostic), explicit deferred items ("advanced color variables" — HSL/OKLCH/OKLab, linked color palettes, prefers-color-scheme conditionals, live preview), and the manual Firefox verification checklist for when the deferred work lands. No parser changes — by design; the slice exists to lock in the baseline before any expansion ships. Verification: `npx vitest run tests/userstyle-compat-fixtures.test.js --pool=vmThreads --maxWorkers=1` passed (18 tests).
...
- [x] Generate Firefox and Edge manifests from the Chrome source manifest. — Shipped 2026-05-24 via `scripts/generate-manifest-firefox.mjs` and `manifest-firefox.transformations.json`; Firefox builds fail on generated-manifest drift, Edge staging uses the shared `edge` profile, and `manifest-firefox.json` remains committed until maintainers approve treating generated manifests as ignored build outputs.
- [x] Ship disabled-by-default Phase R-1 ESM userscript bundling. — Shipped 2026-05-24 via `experimentalESMUserscripts`, parser detection for `@module 1` / `@inject-into module`, offscreen-Acorn static import rewriting, recursive dependency fetch through the existing require path, and dynamic `import()` rejection. Follow-on phases remain R-2 dashboard badge, R-3 receipt/rollback integration detail, R-4 localhost dev SSE, and R-5 default-on decision after dogfooding.
- [x] Collapse runtime JS and TypeScript mirror to one authoritative source. (Design shipped 2026-05-24 in `docs/ts-authoritative-source-design.md`: staged TS promotion with a promotion map, drift gate, `modules/error-log.js` pilot, and final `background.core.js` replacement only after leaf modules are promoted. Gate shipped 2026-05-24 via `ts-source-promotion.json`, `scripts/check-ts-source-drift.mjs`, `npm run ts-source:check`, and CI wiring. First pilot shipped 2026-05-24: `modules/error-log.js` is generated from `src/modules/error-log.ts` via `scripts/generate-ts-runtime-modules.mjs`, `npm run ts-runtime:check` gates drift, and `background.js` is rebuilt from the generated artifact. Shared/low-dependency promotions shipped 2026-05-24: `shared/utils.js`, `modules/notifications.js`, `modules/npm-resolve.js`, `modules/quota-manager.js`, and `modules/userstyles.js` are generated from TS with known drift reconciled or runtime coverage added. Early/storage/resource promotion shipped 2026-05-24 with `modules/i18n.js`, `modules/xhr.js`, `modules/internal-host-guard.js`, `modules/resources.js`, and `modules/storage.js`; generated storage now carries the v3 IndexedDB engine into production while keeping the concatenated service-worker global contract. Sync/import and background-helper promotions continued through 2026-06-04. Completed 2026-06-04: `background.core.js` is generated from `src/background/core.ts`; after the later Sigstore parser/verifier and sync-crypto promotions, `ts-source:check` reports 26 promoted entries, 0 mirrored entries, and 0 intentionally divergent runtime files.)
- [x] Build a full release trust pipeline with signatures, SBOM, source ZIP, package diff, rollback rehearsal, and CWS/AMO status checks. Completed 2026-06-04 with `npm run release:trust`, `npm run release:rollback-drill`, and `npm run release:store-status`; credential custody/OIDC remains a separate hardening item.
