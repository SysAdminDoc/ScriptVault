...
- [x] P1 — Continue TypeScript authoritative-source promotion until `background.core.js` is generated from TS
  - Why: runtime/TS mirror drift is collapsed for the existing concatenated build. `ts-source-promotion.json` now tracks 24 promoted entries, 0 mirrored entries, and 0 intentionally divergent runtime files.
...
- [ ] P2 — Sigstore `@require-provenance` implementation
  ...
  - Progress: 2026-06-04 shipped Phase A parser/storage foundation and Phase B bundle parsing. `@require-provenance` and `@require-identity` now persist as ordered metadata arrays across the main parser and public API install path, trust receipts record declaration-only provenance metadata with `verification: not-yet-implemented`, and `modules/sigstore-bundle-parser.js` is generated from `src/modules/sigstore-bundle-parser.ts` with strict Sigstore v0.3 JSON validation for message-signature and DSSE bundle content. Remaining: signature/root verification, install/details UI, and author guide.
