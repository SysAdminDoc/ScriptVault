...
>
> **Roadmap version:** Round 36 - remaining select validation slice 2026-06-06.
...
> **Source floor:** 400+ external URLs across Rounds 1-36. Every Now/Next item carries source IDs from the Appendix.
>
> Last researched: Round 36 - 2026-06-06.
...
- **Current cycle:** Round 36 Cycle 68 continued N-1 Settings Schema Parity and Accessible Validation. Badge info, blacklist source, config mode, download mode, editor theme, highlight matches, indent style, key mapping, logging level, popup columns, script order, search integration, tab mode, and trash mode now have schema validation metadata, accessible error nodes, and save-blocking allowed-option checks. Focused dashboard a11y/schema tests, settings schema check, TS runtime check, audit, full check suite, build, CWS scan, and `git diff --check` passed.
- **Next implementation angle:** Cycle 69 should continue N-1 by auditing export/backup and other non-select dashboard-saved settings for missing native constraints, text errors, and save-blocking validation; start with settings that accept free-form text, JSON, CSS, file/pattern lists, or numeric values that are not yet covered by validation metadata.
