...
>
> **Roadmap version:** Round 24 - local workspace binding store 2026-06-06.
...
> **Test suite:** 1426 Vitest cases green; `npm audit --audit-level=high --omit=optional` clean; 27/27 TS-promoted runtime entries; 0 mirrored; 0 divergent.
> **Source floor:** 400+ external URLs across Rounds 1-24. Every Now/Next item carries source IDs from the Appendix.
>
> Last researched: Round 24 - 2026-06-06.
...
- **Progress:** Cycle 56 added `LocalWorkspaceBindings` as a local-only IndexedDB-backed store in `modules/storage.js`. It can persist `FileSystemHandle` objects separately from `ScriptStorage`, returns display-safe summaries that omit handles and absolute paths, and is cleaned up when scripts are deleted or storage is cleared. UI binding/refresh actions are not wired yet.
...
- **Current cycle:** Round 24 Cycle 56 implemented the first X-8 local workspace binding store and N-8 redaction fixtures. File handles live in the separate local-only `localWorkspaceBindings` store, summaries omit handles/paths, bindings clean up with scripts, and export/sync/support tests prove future local workspace metadata does not leave through JSON, CloudSync, EasyCloud, or support snapshots.
- **Next implementation angle:** Cycle 57 should continue X-8 by wiring a feature-detected dashboard "Bind local file" action with user-gesture-only `showOpenFilePicker()`, permission-state summaries from `LocalWorkspaceBindings`, and behavior-level no-code/local-save history tests.
