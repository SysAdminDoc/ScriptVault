...
>
> **Roadmap version:** Round 40 - Monaco ESM budget 2026-06-06.
...
> **Test suite:** 1450 Vitest cases green; `npm audit --audit-level=high --omit=optional` clean; 27/27 TS-promoted runtime entries; 0 mirrored; 0 divergent.
...
- **Acceptance for next implementation slice:** Replace the AMD loader block only with a same-change Chromium smoke plan, AMD-removal/static-asset tests, and a fallback test proving `pages/monaco-adapter.js` still activates textarea mode if the ESM bundle is missing.
...
- **Current cycle:** Round 40 Cycle 75 selected the full-worker Chromium Monaco ESM strategy and made it enforceable. `npm run monaco:esm:check` now records gzip sizes and fails if the prototype exceeds the total, compressed, editor, or TypeScript-worker budgets.
- **Next implementation angle:** Cycle 76 should continue X-4 by preparing the actual sandbox switch: update the static package contract for ESM, add AMD-removal tests, and define a Chromium editor smoke/fallback proof before replacing `pages/editor-sandbox.html`.
