...
>
> **Roadmap version:** Round 21 - setup rehydration evidence 2026-06-06.
...
> **Test suite:** 1418 Vitest cases green; `npm audit --audit-level=high --omit=optional` clean; 27/27 TS-promoted runtime entries; 0 mirrored; 0 divergent.
> **Source floor:** 400+ external URLs across Rounds 1-21. Every Now/Next item carries source IDs from the Appendix.
>
> Last researched: Round 21 - 2026-06-06.
...
- **Local evidence:** `pages/dashboard.js` `saveCurrentScript()` still sends `saveScript` with only `markModified: true`; `src/background/core.ts` records a `saveScript` trust receipt only when `data.trust` supplies `recordReceipt`, `operation`, or `sourceUrl`; `src/types/script.ts` already allows `ScriptTrustReceipt.operation === 'local-save'`; `src/types/messages.ts` exposes the `saveScript.trust` object but only for operation/sourceUrl/recordReceipt; `createScriptTrustReceipt()` falls back from empty `sourceUrl` to metadata `source`, `downloadURL`, or `updateURL`, so a local editor save needs an explicit local-source override instead of just passing `sourceUrl: ''`; `src/background/core.ts` `exportAllScripts()` can export script settings and `versionHistory`, cloud export reuses that path, and support snapshots expose only sanitized script inventory fields when the sensitive category is opted in.
...
- **Current cycle:** Round 21 Cycle 53 implemented aggregate registration-sweep evidence for N-7. Local health reports and support snapshots now expose only setup/sweep counts and states, never script names, script IDs, code, or URLs.
- **Next implementation angle:** Cycle 54 should start N-8 local-save trust receipts: extend the save trust payload/source schema, add the dashboard editor-save helper, suppress remote metadata fallback for local saves, and pin manual-save/autosave/export redaction tests.
