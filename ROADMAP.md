...
- [ ] P2 — Sigstore `@require-provenance` implementation
  ...
  - Progress: 2026-06-04 shipped Phase A parser/storage foundation, Phase B bundle parsing, and Phase C message-signature verification. `@require-provenance` and `@require-identity` now persist as ordered metadata arrays, `modules/sigstore-bundle-parser.js` validates Sigstore v0.3 JSON, and `modules/sigstore-bundle-verifier.js` verifies message-signature bundles by hashing the `@require` body, extracting the P-256 public key from the leaf certificate, validating DER/raw ECDSA signatures, and matching URI SAN plus Fulcio issuer extension against `@require-identity`. Trust receipts now opportunistically record `signature-verified`, `signature-failed`, `bundle-unavailable`, or `unsupported-bundle` outcomes when both dependency and bundle fetchers are available. Remaining: Fulcio root/expiry verification, install/details UI, and author guide.
