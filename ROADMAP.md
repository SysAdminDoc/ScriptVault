...
>
> **Roadmap version:** Round 34 - security select validation slice 2026-06-06.
...
> **Source floor:** 400+ external URLs across Rounds 1-34. Every Now/Next item carries source IDs from the Appendix.
>
> Last researched: Round 34 - 2026-06-06.
...
- **Progress:** Schema classification and `npm run settings:schema:check` shipped 2026-06-06. Cycle 62 added S3 sync field validation metadata, native dashboard constraints, accessible error nodes, and blur/save validation for endpoint, region, bucket, and object key. Cycle 63 added sync credential validation metadata, native limits, accessible error nodes, and save-blocking validation for WebDAV URL/username/password, sync encryption passphrase, and S3 access/secret keys; WebDAV URL is required only for WebDAV, S3 access and secret keys are required only for S3, and sync encryption cannot be enabled without a passphrase. Cycle 64 added validation metadata, error nodes, and save-blocking allowed-option checks for editor font size, indentation width, and tab size. Cycle 65 added the same allowed-option validation for update check, notification hide delay, and externals update interval selects, and fixed `0`/"Never" values so they no longer collapse to fallback values. Cycle 66 added validation metadata, error nodes, and save-blocking allowed-option checks for content script API, sandbox mode, CSP modification mode, and HTTP header modification mode. Remaining: continue UI constraints and field-specific errors for action-menu/security selects, export/backup settings, and any other unvalidated dashboard-saved settings.
...
- **Current cycle:** Round 34 Cycle 66 continued N-1 Settings Schema Parity and Accessible Validation. Content script API, sandbox mode, CSP modification mode, and HTTP header modification mode now have schema validation metadata, accessible error nodes, and save-blocking allowed-option checks. Focused dashboard a11y/schema tests, settings schema check, TS runtime check, audit, full check suite, build, CWS scan, and `git diff --check` passed.
- **Next implementation angle:** Cycle 67 should continue N-1 by applying allowed-option validation to the remaining security/action-menu selects whose option lists already exist in schema metadata, starting with default tab types, local file access, cookie access, communication access, SRI, include/check-connect toggles, incognito behavior, page filtering, block behavior, strict mode, and top-level await; then continue export/backup setting constraints with focused dashboard a11y/schema coverage.
