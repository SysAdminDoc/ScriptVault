...
describe('Monaco ESM migration plan', () => {
  it('pins the current AMD state and target ESM bundle shape', () => {
    ...
    expect(plan).toContain('monaco-editor@^0.56.0');
    expect(plan).toContain('Monaco 0.53.0 deprecated AMD support');
    expect(plan).toContain('Monaco 0.56.0');
    expect(plan).toContain('lib/monaco-esm/editor.js');
    expect(plan).toContain('lib/monaco-esm/workers/');
  });

  it('pins the Monaco 0.56 namespace contract used by the sandbox', () => {
    const packageJson = JSON.parse(read('package.json'));
    const packageLock = JSON.parse(read('package-lock.json'));
    // Monaco 0.56 removed editor.main.d.ts and moved the top-level `lsp` /
    // `typescript` namespace exports into the package entry (esm/vs/index.d.ts).
    const editorTypes = read('node_modules/monaco-editor/esm/vs/index.d.ts');
