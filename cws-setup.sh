...
echo "You need 5 values for chrome-webstore-upload-cli v4 (CWS API v2)."
echo "Follow the steps below to get them."
echo ""
...
# ── Step 2: Publisher ID ─────────────────────────────────────────────────────
# New in chrome-webstore-upload-cli v4 (CWS API v2). Required by every upload
# and publish call. Find it under the developer-dashboard Account / Settings
# page (not the per-extension page).
echo "--- STEP 2: Publisher ID ---"
echo "Required by chrome-webstore-upload-cli v4 (CWS API v2)."
echo "Find it at: https://chrome.google.com/webstore/devconsole (Account / Settings page)"
echo ""
read -p "Publisher ID: " PUBLISHER_ID
echo ""

# ── Step 3: OAuth Client ID & Secret ─────────────────────────────────────────
echo "--- STEP 3: Google OAuth2 Credentials ---"
...
echo "--- STEP 4: Authorization & Refresh Token ---"
echo ""
...
# Format matches chrome-webstore-upload-cli v4 (CWS API v2) — secrets are
# env-only; PUBLISHER_ID is required.
...
echo "  bash publish.sh           # Upload + publish"
